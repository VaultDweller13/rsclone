...
function enterCastle() {
  ...
  document.getElementById('tavern')?.addEventListener('click', () =>
    {
      view.append(createLayer(1, createChoice('tav-choice',[{id: 'add-to-prty', name: 'add to party'},
       {id: 'rmv-frm-prty', name: 'remove from party'}, {id: 'inspect', name: 'inspect character'}])));
    });
  ...
  document.getElementById('trade-post')?.addEventListener('click', () => {
    view.append(createLayer(1, createChoice('tav-choice',[
      {id: 'buy', name: 'buy'},
      {id: 'sell', name: 'sell'},
